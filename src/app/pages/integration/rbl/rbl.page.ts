...
@Component({
  selector: 'app-rbl',
  templateUrl: './rbl.page.html',
  styleUrls: ['./rbl.page.scss'],
})
export class RBLPage implements OnInit {
  ...
  ngOnInit() {
    this.provider = this.integrationVisibility.getProvider('rbl');
    this.enabled = this.integrationVisibility.getProviderState('rbl').enabled;
  ...
  async onToggle(event: any): Promise<void> {
    ...
    await firstValueFrom(this.integrationVisibility.saveProviderState('rbl', enable));
    this.enabled = enable;
    await this.uiToast.show(enable ? 'RBL visibility enabled.' : 'RBL visibility disabled.', 'primary');
  }
